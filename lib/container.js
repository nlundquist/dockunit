...
Container.prototype.startAndMountContainer = function(options, finishedCallback) {
	function windowsDockerPath(winPath) {
	...
	var containerId,
		self = this,
		path = isWin ? windowsDockerPath(global.config.path) : global.config.path,
		run = spawn('docker', ['run', '-d', '-v', path + ':/app/test', '-w', '/app/test', '-it', self.image, '/bin/bash']);
	
	// Dump errors to the stderr output so they can be seen.
	run.stderr.on('data', function(data) {
		console.error('ERROR: ', data.toString());
		console.error('COMMAND: ', 'docker', 'run', '-d', '-v', path + ':/app/test', '-w', '/app/test', '-it', self.image, '/bin/bash');
	});
...
Container.prototype.backupFiles = function(options, containerId, finishedCallback) {
	...
	var containerCmd = ['bash','-c','source ~/.bashrc && cp -p -r /app/test /app/backup'],
		exec = spawn('docker', ['exec', containerId].concat(containerCmd));
...
Container.prototype.runBeforeScript = function(options, index, containerId, finishedCallback) {
	...
	spawn('docker', ['exec', containerId].concat(self.beforeScripts[index].split(' ')), options).on('exit', function(code) {
		if (0 !== code) {
...
Container.prototype.runTests = function(options, containerId, finishedCallback) {
    ...
    var containerCmd = ['bash','+','-c','source ~/.bashrc && ' + self.testCommand + ' ' + testArgString, '1>&2'],
        exec = spawn('docker', ['exec', containerId].concat(containerCmd), { stdio: 'inherit' }).on('exit', function(code) {
            if (code !== 0) {
...
Container.prototype.cleanupFiles = function(options, containerId, finishedCallback) {
	...
	var containerCmd = ['bash', '-c', 'source ~/.bashrc && rm -rf /app/test/* && cp -p -r /app/backup/* /app/test', '1>&2'],
		exec = spawn('docker', ['exec', containerId].concat(containerCmd), { stdio: 'inherit' }).on('exit', function(code) {
		if (code !== 0) {
...
/**
 * Open console in container for test debugging purposes
 *
 * @param options
 * @param containerId
 * @param finishedCallback
 */
Container.prototype.openConsole = function(options, containerId, finishedCallback) {
	var debug = spawn('docker', ['exec', '-it', containerId, 'bash'], {stdio: 'inherit'});

	console.log('\nDebugging Started'.green);

	debug.on('exit', function() {
		if (global.config.verbose) {
			console.log('\nDebugging Finished'.green);
		}
